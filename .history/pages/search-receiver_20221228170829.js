...
function SearchReceiver() {
  ...
  useEffect(() => {
    getListTransaction().then((data) => {
      setListTransaction(data.results);
    });
  }, []);

  const getListTransaction = async () => {
    const { data } = await http(token).get("https://68xkph-8888.preview.csb.app/transactions?page=1&limit=5");
    return data;
  };
            ...
            <div>
              {listTransaction.map((bio) => (
                ...
                <div className="flex mb-[60px] border-1 shadow-md p-[20px] rounded-[10px]">
                  ...
                  <div className="flex-1">
                    <p className="text-[#4D4B57] text-[16px] leading-[21px] font-bold mb-[9px]" key={id}>
                      {listTransaction.firstName + "  " + listTransaction.lastName}
                    </p>
                    <p className="text-[#7A7886] text-[14px] leading-[19px]">{listTransaction.phoneNumber}</p>
                  </div>
                </div>
              ))}
              {/* <div className="flex mb-[60px] border-1 shadow-md p-[20px] rounded-[10px]">
