              ...
              <div className="flex flex-col">
                {notification.recipientId === userId && notification.sendername ? (
                  <div className="flex">
                    ...
                    <span className=" font-bold text-[16px] leading-[21px] text-[#1EC15F] ">Accept from {list.recipientname}</span>
                  </div>
                ) : (
                  <div className="flex">
                    <Image src={arrowRed} alt="up-arrow" className="w-[30px] h-[30px] mr-3" />
                    <span className=" font-bold text-[16px] leading-[21px] text-red-500 ">Transfer to {list.recipientname}</span>
                  </div>
                )}
                <span className="text-right text-[#43484F] text-[18px] font-bold">IDR.{list.amount}</span>
              </div>
            </div>
          ))}
        </div>
